$(document).ready(function() {
  ...
  function updateAllGraphs(){
    ...
    loadViolinChart("Humidity", current_graphs);
  
    loadViolinChart("CO2", current_graphs);
  
    loadViolinChart("PM25", current_graphs);
  
    loadViolinChart("Temperature", current_graphs);
  
    loadViolinChart("VOC", current_graphs);
      ...
      function loadChart(graph_type,filter_type){
        ...
        if(include_suspicious_data){
          var sus_url = "?resolution=" + json_resolution_value;
        } else {
          sus_url = "?suspiciousFilter=suspicious&resolution=" + json_resolution_value;
        }
  
        if(graph_type == "VOC"){
          json_url = url + "/graph_php_files/graph_hub_sql.php?isVOC=yes&resolution=" + json_resolution_value;
        } else {
  ...
  var layout = {
    ...
    Plotly.newPlot('myDiv-' + graph_type, large_room_data, layout);
     ...
     function loadViolinChart(graph_type,filter_type){
        ...
        if(include_suspicious_data){
          ...
          sus_url = "suspiciousFilter=suspicious&";
        }
  
        if(graph_type == "VOC"){
          json_url = url + "/graph_php_files/graph_hub_sql.php?isVOC=yes&schoolTime=yes&resolution=" + json_resolution_value;
        } else {
          json_url = url + "/graph_php_files/graph_hub_sql.php?" + sus_url + "schoolTime=yes&resolution=" + json_resolution_value;
        }
